...
internal sealed class Argon2idCore
{
    ...
    internal class PseudoRands2i(ulong[] rands) : IArgon2PseudoRands
    {
        private readonly ulong[] _rands = rands;
        public ulong PseudoRand(int segment, int prevLane, int prevOffset) => _rands[segment];
    }
    internal class PseudoRands2d(Argon2Lane[] lanes) : IArgon2PseudoRands
    {
        private readonly Argon2Lane[] _lanes = lanes;
        public ulong PseudoRand(int segment, int prevLane, int prevOffset) => _lanes[prevLane][prevOffset].Span[0];
    }
    private const uint ARGON2_SYNC_POINTS = 4;
    private readonly static Memory<ulong> _zeroBlock = new(new ulong[128]);
    internal IArgon2PseudoRands GenerateState(Argon2Lane[] lanes, int segmentLength, int pass, int lane, int slice)
    {
        if ((pass == 0) && (slice < (ARGON2_SYNC_POINTS / 2)))
        {
            ...
            return new PseudoRands2i(rands);
        }
        return new PseudoRands2d(lanes);
    }
